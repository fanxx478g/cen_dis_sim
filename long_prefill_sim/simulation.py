...
class PrefillConfig:
    ...
    long_prefill_min_tokens: int = 32 * 1024
...
class PreparedSimulation:
    ...
    region_first_central_region_index: int | None
    region_first_outstanding_threshold: int
    seed: int
...
def prepare_simulation_config(config: SimulationConfig) -> PreparedSimulation:
    ...
    if config.region_policy not in (
        ...
        raise ValueError(
            "region_policy 必须是 weight_region_policy、queue_len_policy、"
            "d_queue_len_policy 或 region_first_policy。"
        )
    ...
    if config.region_first_outstanding_threshold <= 0:
        raise ValueError("region_first_outstanding_threshold 必须大于 0。")
    ...
    if prefill.long_prefill_min_tokens < prefill.truncated_min_tokens:
        raise ValueError("prefill.long_prefill_min_tokens 不能小于 truncated_min_tokens。")
    if prefill.long_prefill_min_tokens > prefill.truncated_max_tokens:
        raise ValueError("prefill.long_prefill_min_tokens 不能大于 truncated_max_tokens。")
    ...
    central_region_index = config.region_first_central_region_index
    if central_region_index is not None:
        if central_region_index < 0 or central_region_index >= len(region_names):
            raise ValueError("region_first_central_region_index 超出 region_names 范围。")
    if config.region_policy == "region_first_policy":
        if central_region_index is None:
            raise ValueError("region_first_policy 需要设置 region_first_central_region_index。")
        if users_by_region[central_region_index] != 0:
            raise ValueError(
                "region_first_policy 要求 central region 不直接承载 home 用户，请将该地域的 "
                "users_by_region 设为 0。"
            )
    ...
    return PreparedSimulation(
        ...
        region_first_central_region_index=central_region_index,
        region_first_outstanding_threshold=int(config.region_first_outstanding_threshold),
...
def prepare_deployment_config(
    ...
    if simulation.region_policy == "region_first_policy" and deployment_config.mode != "distributed":
        raise ValueError("region_first_policy 只能用于 distributed 模式。")
    ...
    if simulation.region_policy == "region_first_policy":
        central_region_index = simulation.region_first_central_region_index
        if central_region_index is None:
            raise ValueError("region_first_policy 缺少 central region 配置。")
        if len(instance_counts) < 2:
            raise ValueError("region_first_policy 至少需要 1 个本地域和 1 个 central region。")
        if instance_counts[central_region_index] <= 0:
            raise ValueError("region_first_policy 的 central region 实例数必须大于 0。")
...
def generate_workload(config: SimulationConfig | PreparedSimulation) -> Workload:
    ...
    request_nonzero_prefill_tokens = _sample_long_prefill_tokens(
...
def expected_prefill_ms(config: SimulationConfig | PreparedSimulation) -> float:
    ...
    lower = math.log(prefill.long_prefill_min_tokens)
    upper = math.log(prefill.truncated_max_tokens)
...
def _initialize_user_region_bindings(
    ...
    if deployment.mode == "distributed" and simulation.region_policy == "region_first_policy":
...
def _choose_region_first_execution_region(
    home_region_index: int,
    central_region_index: int,
    queue_lengths: np.ndarray,
    in_service_counts: np.ndarray,
    outstanding_threshold: int,
) -> int:
    """本地优先，排队或达到阈值后回退到 central region。"""

    if home_region_index == central_region_index:
        raise ValueError("home_region_index 不能等于 central_region_index。")

    local_waiting = int(queue_lengths[home_region_index])
    local_outstanding = int(queue_lengths[home_region_index] + in_service_counts[home_region_index])
    if local_waiting > 0 or local_outstanding >= outstanding_threshold:
        return central_region_index
    return home_region_index


def _pop_distributed_busy_request(
    busy_requests: list[list[PendingRequest | None]],
    user_id: int,
    request_index: int,
    instance_id: int,
) -> tuple[int, PendingRequest]:
    """按 completion 事件定位实际执行该请求的地域并弹出。"""

    for region_index, region_busy_requests in enumerate(busy_requests):
        if instance_id >= len(region_busy_requests):
            continue
        pending = region_busy_requests[instance_id]
        if pending is None:
            continue
        if pending.user_id == user_id and pending.request_index == request_index:
            region_busy_requests[instance_id] = None
            return region_index, pending
...
def _run_event_loop(
    ...
    if deployment.mode == "centralized":
        ...
        while event_heap:
            event_time, _, event_type, user_id, request_index, instance_id = heapq.heappop(event_heap)
            user_index = user_id - 1
            home_region_index = int(user_region_bindings[user_index])
            ...
            if event_type == "arrival":
                region_index = home_region_index
                if simulation.region_policy == "region_first_policy":
                    central_region_index = simulation.region_first_central_region_index
                    if central_region_index is None:
                        raise RuntimeError("region_first_policy 缺少 central region 配置。")
                    region_index = _choose_region_first_execution_region(
                        home_region_index=home_region_index,
                        central_region_index=central_region_index,
                        queue_lengths=queue_lengths,
                        in_service_counts=in_service_counts,
                        outstanding_threshold=simulation.region_first_outstanding_threshold,
                    )
                elif region_index < 0:
            ...
            region_index, pending = _pop_distributed_busy_request(
                busy_requests=busy_requests,
                user_id=user_id,
                request_index=request_index,
...
def _sample_long_prefill_tokens(
    request_count: int,
    simulation: PreparedSimulation,
    rng: np.random.Generator,
) -> np.ndarray:
    ...
    while filled < request_count:
        ...
        valid = sampled[
            (sampled >= prefill.long_prefill_min_tokens)
            & (sampled <= prefill.truncated_max_tokens)
        ]
        if valid.size == 0:
            continue
